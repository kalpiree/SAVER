...
class Trace(contextlib.AbstractContextManager):
    ...
    def __init__(
        ...
        # The hook signature in that mode is (module, args, kwargs, output).
        try:
            def kwargs_hook(m, inputs, kwargs, output):
                return retain_hook(m, inputs, output, kwargs=kwargs)

            self.registered_hook = module.register_forward_hook(kwargs_hook, with_kwargs=True)
        except TypeError:
